...
dataset_names = {'Flevoland'};
size_method_names=size(method_names);
svm_para = { '-c 10 -g 0.125 -m 500 -t 2 -q'}; % Parameters to perform svm. You should install LibSVM toolbox first.
...
% Feature selection for Flevoland dataset
for dataset_id = [1]
    % Load data
    [Dataset]= get_data_Flevoland(dataset_names{dataset_id});
      ...
      % Update the training and testing samples
        ...
        for classifier_id = update_classifier_ids{dataset_id}
            ...
            if find(updated_method_ids == 1)
                cur_accu = test_fs_accu_Flevoland(Methods{1, 1}.feature_set{dataset_id, 1}, Dataset, classifier_names{classifier_id});
            ...
            for j = updated_method_ids
                ...
                for k = x
                    ...
                    cur_accu = test_fs_accu_Flevoland(Methods{1, j}.feature_set{dataset_id, cnt}, Dataset, classifier_names{classifier_id});
