...
% Feature selection for S1AFS dataset
for dataset_id = [1]
    % Load data
    [Dataset]= get_data_S1AFS(dataset_names{dataset_id});
      ...
      % Update the training and testing samples
        ...
        for classifier_id = update_classifier_ids{dataset_id}
            ...
            if find(updated_method_ids == 1)
                cur_accu = test_fs_accu_S1AFS(Methods{1, 1}.feature_set{dataset_id, 1}, Dataset, classifier_names{classifier_id});
            ...
            for j = updated_method_ids
                ...
                for k = x
                    ...
                    if(k==x(1))
                        if(ite==1)
                            if(classifier_id == 2)
                                % Only 3-dimension features are used to generate classification maps. Use KNN to plot classification maps.
                                %imaging(Methods{1, j}.feature_set{dataset_id, cnt}, Dataset, j, method_names);
                            end
                        end
                    end
                    cur_accu = test_fs_accu_S1AFS(Methods{1, j}.feature_set{dataset_id, cnt}, Dataset, classifier_names{classifier_id});
