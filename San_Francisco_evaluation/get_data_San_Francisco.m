function [Dataset,pixel_pos] = get_data_San_Francisco(dataset_name)
    % load data
    disp(['Load ' char(dataset_name) ' dataset...'])
    load('.\data\San_Francisco_all_metrics_normalized.mat');
    A = all_metrics_normalized;
    A(find(isnan(A)==1)) = 0;
    load('.\data\San_Francisco_GT.mat');
    ground_truth = san_francisco_GT;
